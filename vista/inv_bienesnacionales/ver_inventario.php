 ...
 function buscar(id)
 {
      ...
      if(confirm("¿Esta seguro que desea desactivar el docente seleccionado?"))
      {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                if(Confi.lsExito==1)
                {
                    Notifica_Logro("Recepción desactivada con éxito.");                         
                }
                else    
                {
                    Notifica_Error("No se puede desactivar esta recepción, inténtelo mas tarde.");
                }
  ...
  function restaurar(id)
  {
     if(confirm("¿Esta seguro que desea restaurar el docente seleccionado?"))
    {
      ...
      document.form_recepcion.submit();
    }
  }
...
<div style="float: left" class="col-lg-10 span10 pull-left">
    ...
    <form action="../controlador/cConsultasAjaxBN.php" method="POST" id="form_recepcion" name="form_recepcion">
        ...
        <table class="table table-striped table-hover table-bordered bootstrap-datatable datatable dataTable" id="filtro">
            <thead>
                <th>N° Recepción</th><th>Fecha Llegada</th><th>Fecha de Registro</th><th>Proveedor</th><th>Responsable</th><th>Usuario</th><th>Motivo</th><th>Observación</th><th>Acción</th>
            </thead>
            <tbody>
            <?php
                    require_once('../clases/update2016/clase_recepcionbn.php');
                    $loRecepciones=new clsRecepcion;
                    $laArticulosBN=$loRecepciones->listarRecepciones();
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="myModalLabel">Descactivar Recepción</h4>
                        </div>
...
<script>
    $('#ModalDeleteElem').on('shown.bs.modal', function () {
        $('#myInput').focus()
      })
</script>