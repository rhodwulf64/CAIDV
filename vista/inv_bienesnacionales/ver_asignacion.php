      ...
      if(confirm("¿Esta seguro que desea desactivar el docente seleccionado?"))
      {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                if(Confi.lsExito==1)
                {
                    Notifica_Logro("Asignación desactivada con éxito.");                         
                }
                else    
                {
                    Notifica_Error("No se puede desactivar esta asignación, inténtelo mas tarde.");
                }
...
<div style="float: left" class="col-lg-10 span10 pull-left">
    ...
    <form action="../controlador/cConsultasAjaxBN.php" method="POST" id="form_recepcion" name="form_recepcion">
        ...
        <table class="table table-striped table-hover table-bordered bootstrap-datatable datatable dataTable" id="filtro">
            <thead>
                <th>N° Asig.</th><th>Fecha Asig.</th><th>Fecha de Registro</th><th>Departamento</th><th>Responsable Departamento</th><th>Responsable Asig.</th><th>Usuario</th><th>Motivo</th><th>Observación</th><th>Acción</th>
            </thead>
            <tbody>
            <?php
                    require_once('../clases/update2016/clase_asignacionbn.php');
                    $loAsignaciones=new clsAsignacion;
                    $laArticulosBN=$loAsignaciones->listarAsignaciones();
                    ...
                    for($i=0;$i<count($laArticulosBN);$i++)
                    {
                        ...
                        echo '<td>'.$laArticulosBN[$i][24].'</td>';
                        echo '<td>'.$laArticulosBN[$i][30].'</td>';
