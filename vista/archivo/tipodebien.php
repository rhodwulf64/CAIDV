...
function fpAnalizaExistencia($valor)
{
    $retorno=$valor;
    if ($valor=="")
    {
...
for($i=0;$i<count($laModulos);$i++) 
    {
        ...
        {
            if($laServicios[$j][2]=='archivo/consultar_tipodebien')
            {
                $consultar=true;
            }
            if($laServicios[$j][2]=='archivo/registrar_tipodebien')
            {
                $registrar=true;
            }
            if($laServicios[$j][2]=='archivo/eliminar_tipodebien')
            {
                $eliminar=true;
            }
        }
...
<script>
 function buscar(id)
 {
    window.location.href="?vista=archivo/registrar_tipodebien&id="+id;
 }
  ...
  function descativar()
  {
    ...
    {
        if(confirm("¿Esta seguro que desea desactivar el Tipo de Bien seleccionado?"))
        {
  ...
  {
     if(confirm("¿Esta seguro que desea restaurar el Tipo de Bien seleccionado?"))
    {
      document.getElementById("cam_idinstrumento").value=id;
      document.getElementById("cam_operacion").value='restaurar_tipodebien';
      document.form_config.submit();
    }
  }
</script>
...
<div style="float: left" class="col-lg-10 span10 pull-left">
    <h3>Tipo de Bien Nacional</h3>
    <div class="alert alert-info">
        <ul>
            En este módulo podrá registrar, consultar, editar, desactivar y activar los Tipos de Bienes Nacionales.
        </ul>
    </div>
    <form action="../controlador/control_tipodebien.php" method="POST" name="form_config">
        <input type="hidden" value="eliminar_tipodebien" name="operacion" id="cam_operacion"/>
        <input type="hidden"  name="idinstrumento" id="cam_idinstrumento"/>        
        <?php
        if($registrar)
        {
            echo '<a id="btn_registrar" class="btn btn-success" href="?vista=archivo/registrar_tipodebien"><i class="icon-plus icon-white"></i> Registrar Tipo de Bien Nacional</a>';
        }
        ?>
        <table class="table table-striped table-hover table-bordered bootstrap-datatable datatable dataTable" id="filtro">
            <thead>
                <th>Código</th><th>Serie</th><th>Descripción</th><th>Categoría</th><th>Existencia</th><th>Estatus</th><?php if($consultar || $eliminar)
                        { echo '<th>Acción</th>';}?>
            </thead>
            <tbody>
            <?php
                    ...
                    $lsConsultaPrincipal=$loPrincipal->consultar_tipodebien_bitacora();
                    for($i=0;$i<count($lsConsultaPrincipal);$i++)
                    {
                        ...
                        echo '<td>'.$lsConsultaPrincipal[$i][4].'</td>';
                        echo '<td>'.$lsConsultaPrincipal[$i][1].'</td>';
                        echo '<td>'.$lsConsultaPrincipal[$i][2].'</td>';
                        echo '<td>'.fpAnalizaExistencia($lsConsultaPrincipal[$i][5]).'</td>';
