...
for($i=0;$i<count($laModulos);$i++) 
    {
        ...
        {
            if($laServicios[$j][2]=='archivo/consultar_modelo')
            {
                $consultar=true;
            }
            if($laServicios[$j][2]=='archivo/registrar_modelo')
            {
                $registrar=true;
            }
            if($laServicios[$j][2]=='archivo/eliminar_modelo')
            {
                $eliminar=true;
            }
        }
...
<script>
 function buscar(id)
 {
    window.location.href="?vista=archivo/registrar_modelo&id="+id;
 }
  ...
  function descativar()
  {
    ...
    {
        if(confirm("¿Esta seguro que desea desactivar el Modelo seleccionado?"))
        {
  ...
  {
     if(confirm("¿Esta seguro que desea restaurar el Modelo seleccionado?"))
    {
      document.getElementById("cam_idinstrumento").value=id;
      document.getElementById("cam_operacion").value='restaurar_modelo';
      document.form_config.submit();
    }
  }
</script>
...
<div style="float: left" class="col-lg-10 span10 pull-left">
    ...
    <div class="alert alert-info">
        <ul>
            En este módulo podrá registrar, consultar, editar, desactivar y activar los Modelos de los Bienes Nacionales.
        </ul>
    </div>
    <form action="../controlador/control_modelo.php" method="POST" name="form_config">
        <input type="hidden" value="eliminar_modelo" name="operacion" id="cam_operacion"/>
        <input type="hidden"  name="idinstrumento" id="cam_idinstrumento"/>        
        <?php
        if($registrar)
        {
            echo '<a id="btn_registrar" class="btn btn-success" href="?vista=archivo/registrar_modelo"><i class="icon-plus icon-white"></i> Registrar Modelo</a>';
        }
        ?>
        <table class="table table-striped table-hover table-bordered bootstrap-datatable datatable dataTable" id="filtro">
            <thead>
                <th>Código</th><th>Descripción</th><th>Marca</th><th>Estatus</th><?php if($consultar || $eliminar)
                        { echo '<th>Acción</th>';}?>
            </thead>
            <tbody>
            <?php
                    require_once('../clases/Update2016/clase_modelo.php');
                    $loPrincipal=new clsModelo;
                    $lsConsultaPrincipal=$loPrincipal->consultar_modelo_bitacora();
