...
$loCategoriadebien->set_categoriadebien($id);
$Datos_Instrumento = $loCategoriadebien->consultar_categoriadebien_especifico();
if($Datos_Instrumento)
{
    $operacion='editar_categoriadebien';
    $titulo   ='Editar Categoría de Bien';
}
else
{
    $operacion='registrar_categoriadebien';
    $titulo   ='Registrar Categoría de Bien';
}
?>
<div style="float: left" class="col-lg-8 span8 pull-left">
    ...
    <div class="alert alert-info">
     <ul>
    <li>En este formulario podrá registar las distintas categorías de los diferentes bienes.
    ...
    <form class="formulario" action="../controlador/control_categoriadebien.php" method="POST" name="form_instrumento">
        <input type="hidden" value="<?php print($operacion);?>" name="operacion" />
        <input type="hidden" value="<?php print($Datos_Instrumento['id_categoria']);?>" name="idinstrumento" />
        <div class="row-fluid">
            <div class="col-lg-6 span6">
                <label>Descripción <span class="label label-warning" data-trigger="hover" data-container="body" data-toggle="popover" data-placement="right" data-content="Nombre del bien"><i class="fa fa-question" ></i></span></label>
                <input type="text" class="span12" maxlength="50" name="txtDescripcion" id="txtDescripcion" value="<?php print($Datos_Instrumento['nom_cat']);?>" required/>
            </div>            
        </div>
        <div class="botonera">
            <input type="submit" class="btn btn-success" name="btn_enviar" id="btn_enviar" onclick="return validar();" value="Aceptar">
            <input type="button" class="btn btn-danger" name="btn_regresar" id="btn_regresar" value="Regresar" onclick="window.location.href='?vista=archivo/categoriadebien';">
        </div>
    </form>
</div>
