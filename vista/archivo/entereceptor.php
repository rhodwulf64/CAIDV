...
for($i=0;$i<count($laModulos);$i++) 
    {
        ...
        {
            if($laServicios[$j][2]=='archivo/consultar_entereceptor')
            {
                $consultar=true;
            }
            if($laServicios[$j][2]=='archivo/registrar_entereceptor')
            {
                $registrar=true;
            }
            if($laServicios[$j][2]=='archivo/eliminar_entereceptor')
            {
                $eliminar=true;
            }
        }
...
<script>
 function buscar(id)
 {
    window.location.href="?vista=archivo/registrar_entereceptor&id="+id;
 }
  ...
  function descativar()
  {
    ...
    {
        if(confirm("¿Esta seguro que desea desactivar el Ente Externo seleccionado?"))
        {
...
<div style="float: left" class="col-lg-10 span10 pull-left">
    ...
    <form action="../controlador/control_entereceptor.php" method="POST" name="form_config">
        <input type="hidden" value="eliminar_entereceptor" name="operacion" id="cam_operacion"/>
        ...
        <table class="table table-striped table-hover table-bordered bootstrap-datatable datatable dataTable" id="filtro">
            <thead>
                <th>Código</th><th>Razón Social</th><th>Rif</th><th>Teléfono</th><th>Estatus</th><?php if($consultar || $eliminar)
                        { echo '<th>Acción</th>';}?>
            </thead>
            <tbody>
            <?php
                    ...
                    $lsConsultaPrincipal=$loPrincipal->consultar_entereceptor_bitacora();
