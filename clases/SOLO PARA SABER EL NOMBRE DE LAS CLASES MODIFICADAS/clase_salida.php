<?php
	...
	class clssalida extends ModeloConect
	{
		...
		{
			$this->conectar();
			$sql="INSERT INTO tsalida (cod_tdepartamento,fecha,cedrecibido)VALUES('$this->lccod_tdepartamento',STR_TO_DATE('$this->lcfecha', '%Y-%m-%d'),'$this->lcpersona')";
		...
		function consultar_salida()
		{
			$this->conectar();
			$cont=0;
				$sql="SELECT tsalida.idsalida,tdepartamento.nombre_dep,date_format(fecha,'%d-%m-%Y') as fecha,tsalida.estatus,tpersona.nombre_per,tpersona.apellido_per,tarticulo.descripcionart,dsalida.cantidad FROM tsalida,tdepartamento,dsalida,tarticulo,tpersona WHERE  tpersona.cedula=tdepartamento.cedula and tdepartamento.cod_tdepartamento=tsalida.cod_tdepartamento and tsalida.idsalida='$this->lcIdsalida' and dsalida.idsalida=tsalida.idsalida and tarticulo.idarticulo=dsalida.idarticulo";
				$pcsql=$this->filtro($sql);
				while($laRow=$this->proximo($pcsql))
				{
					$Fila[0]=$laRow['idsalida'];
					$Fila[2]=$laRow['nombre_dep'];
					$Fila[3]=$laRow['fecha'];
					$Fila[4]=$laRow['estatus'];
					$Fila[5]=$laRow['nombre_per'];
					$Fila[6]=$laRow['descripcionart'];
					$Fila[7]=$laRow['cantidad'];
					$Fila[8]=$laRow['apellido_per'];
		...
		function consultar_salida_fin()
		{
			$this->conectar();
			$cont=0;
				$sql="SELECT tsalida.idsalida,tdepartamento.nombre_dep,date_format(fecha,'%d-%m-%Y') as fecha,tsalida.estatus,tpersona.nombre_per,tpersona.apellido_per,tarticulo.descripcionart,dsalida.cantidad,tsalida.observacion,tsalida.fechaentrega,tsalida.nomentrega FROM tsalida,tdepartamento,dsalida,tarticulo,tpersona WHERE  tpersona.cedula=tdepartamento.cedula and tdepartamento.cod_tdepartamento=tsalida.cod_tdepartamento and tsalida.idsalida='$this->lcIdsalida' and dsalida.idsalida=tsalida.idsalida and tarticulo.idarticulo=dsalida.idarticulo";
				$pcsql=$this->filtro($sql);
				while($laRow=$this->proximo($pcsql))
				{
					$Fila[0]=$laRow['idsalida'];
		...
		function consultar_persona()
		{
			$this->conectar();
			$cont=0;
				$sql="SELECT tpersona.nombre_per,tpersona.apellido_per FROM tsalida,tpersona WHERE  tpersona.cedula=tsalida.cedrecibido and tsalida.idsalida='$this->lcIdsalida'";
				$pcsql=$this->filtro($sql);
				while($laRow=$this->proximo($pcsql))
				{
					$Fila[0]=$laRow['nombre_per'];
					$Fila[1]=$laRow['apellido_per'];
					$cont++;
				}
			$this->desconectar();
			return $Fila;
		...
		function consultar_salida_imprimir()
		{
			$this->conectar();
			$cont=0;
				$sql="SELECT idsalida,date_format(fecha,'%d-%m-%Y') as fecha,date_format(fechaentrega,'%d-%m-%Y') as fechaentrega,tdepartamento.nombre_dep,nomentrega,tpersona.apellido_per,tpersona.nombre_per,observacion,tsalida.estatus  FROM tsalida,tdepartamento,tpersona WHERE tsalida.estatus='2' and tdepartamento.cod_tdepartamento=tsalida.cod_tdepartamento and tpersona.cedula=tsalida.cedrecibido";
				$pcsql=$this->filtro($sql);
				while($laRow=$this->proximo($pcsql))
				{
					$Fila[0]=$laRow['idsalida'];
					...
					$Fila[4]=$laRow['nombre_dep'];
					$Fila[5]=$laRow['nomentrega'];
					$Fila[6]=$laRow['apellido_per'];
					$Fila[7]=$laRow['nombre_per'];
		...
		function consultar_salidas_inactivos()
		{
			$this->conectar();
			$cont=0;
				$sql="SELECT idsalida,tdepartamento.nombre_dep,date_format(fecha,'%d-%m-%Y') as fecha,tsalida.estatus  FROM tsalida,tdepartamento WHERE tdepartamento.cod_tdepartamento=tsalida.cod_tdepartamento";
				$pcsql=$this->filtro($sql);
				while($laRow=$this->proximo($pcsql))
				{
					$Fila[$cont][0]=$laRow['idsalida'];
					$Fila[$cont][2]=$laRow['nombre_dep'];
