...
class clsDevolucion Extends ModeloConect{
	private $G;
	...
	function incluir(){
		...
		NULL,
		NULORD('".$this->G["txtDepartamento"]."'),
		NULORD('".$this->G["txtResponsableDto"]."'),
		...
		{ //pregunto si se realizo la consulta anterior
			...
			$this->lcIDdocumento=$idMovimiento;
	 		for($i=1;$i<=$this->G["txtFila"];$i++)
		 	{
		 		...
		 		$sql = "UPDATE articulobn SET id_cond='1' WHERE id_bien='".$this->G["txtCodeInsti".$i]."'";
		 		$this->ejecuta($sql);
	...
	function ConsulCabDev($idDev){
		$sql = "SELECT mv.observacion_mov FROM movimientobn as mv WHERE id_mov='".$idDev."'";
	...
	function ListameBNporTipo()
	{
		...
		INNER JOIN dmovimientobn AS dmov ON artibn.id_bien=dmov.id_bien
		INNER JOIN movimientobn AS mov ON dmov.id_mov=mov.id_mov
		WHERE artibn.status='1' AND artibn.id_cond='2' AND dmov.status='1' AND mov.status='1' AND mov.id_dep='".$this->G["txtDepartamento"]."'
		GROUP BY artibn.id_bien
	...
	function anularDevolucion($idDev,$MotAnulacion){
		...
		if( $this->como_va() ){
			/* inserto los datos correspondiente de la anulacion en la tabla movimientobns */
			$sql = "UPDATE movimientobn SET id_usuario_anulacion='".$_SESSION["id_usuario"]."', fecha_anulacion='".$fecha."', id_motivo_anulacion='".$MotAnulacion."' WHERE id_mov='".$idDev."'";
			$this->ejecuta($sql);

			if( $this->como_va() ){

				/* actulizo el status en la tabla detalle del movimientobn */
				$sql = "UPDATE dmovimientobn SET status='0' WHERE id_mov='".$idDev."'";
				$this->ejecuta($sql);
				...
				if( $this->como_va() ){
					...
					$sql = "SELECT id_bien FROM dmovimientobn WHERE id_mov='".$idDev."'";
					$rs = $this->ejecuta($sql);
					if( $this->como_va() ){
	...
	function consultar_devolucion_bitacora()
	{
				$this->conectar();
		$cont=0;
		$sql = "SELECT
		...
		WHERE mov.id_mov='$this->lcIDdocumento' AND mov.status='1' AND arti.status='1' AND dmov.status='1' AND mov.id_tipo_mov='3'
