...
class clsAsignacion Extends ModeloConect{
	...
	function incluir(){
		...
		/***********************************************/
		$sql="SELECT COUNT(m.nro_document) AS nro FROM movimientobn AS m INNER JOIN motivobn AS mo ON(m.id_motivo_mov=mo.id_motivo_mov) WHERE mo.tipo_motivo=3 AND mo.status=1";
		$rs = $this->ejecuta($sql);
		...
		{ //pregunto si se realizo la consulta anterior
			...
			$this->lcIDdocumento=$idMovimiento;
	 		for($i=1;$i<=$this->G["txtFila"];$i++)
		 	{
		 		...
		 		$sql = "UPDATE articulobn SET id_cond='2' WHERE id_bien='".$this->G["txtCodeInsti".$i]."'";
		 		$this->ejecuta($sql);
	...
	function BuscarAsignacionExiste($cod)
	{
		$result=false;
		$sql = "SELECT
		id_mov,
	...
	function ListameBNporTipo()
	{
		...
		LEFT JOIN marcabn AS mcabn ON artibn.id_marca=mcabn.id_marca
		LEFT JOIN modelobn AS mdlbn ON artibn.id_modelo=mdlbn.id_modelo
	...
	function consultar_asignacion_bitacora()
	{
		$this->conectar();
		$cont=0;
		$sql = "SELECT
		...
		WHERE mov.id_mov='$this->lcIDdocumento' AND mov.status='1' AND arti.status='1' AND dmov.status='1' AND mov.id_tipo_mov='2'
