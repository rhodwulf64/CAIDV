...
class clsDesincorporacion Extends ModeloConect{
	private $G;
	...
	function incluir(){
		...
		$sql="SELECT COUNT(m.nro_document) AS nro FROM movimientobn AS m INNER JOIN motivobn AS mo ON(m.id_motivo_mov=mo.id_motivo_mov) WHERE mo.tipo_motivo=5 AND mo.status=1";
		$rs = $this->ejecuta($sql);
		...
		{ //pregunto si se realizo la consulta anterior
			...
			$this->lcIDdocumento=$idMovimiento;
	 		for($i=1;$i<=$this->G["txtFila"];$i++)
		 	{
		 		...
		 		$sql = "UPDATE articulobn SET id_cond='3' WHERE id_bien='".$this->G["txtCodeInsti".$i]."'";
		 		$this->ejecuta($sql);
	...
	function Consultar_Cant_Movimientos(){
		$sql = "SELECT count(id_mov) as cantidad FROM movimientobn WHERE id_tipo_mov=4 and status=1";
		$rs = $this->ejecuta($sql);
		if( $this->como_va() ){
			$tupla = $this->converArray($rs);
			$cant = $tupla["cantidad"];
	...
	function ConsulCabRece($idDesin){
		$sql = "SELECT mv.observacion_mov FROM movimientobn as mv WHERE id_mov='".$idDesin."'";
	...
	function BuscarDesincorporacionExiste($cod){
		$result=false;
		$sql = "SELECT
		id_mov,
		nro_document,
	...
	function anularDesincorporacion($idDesin,$MotAnulacion){
		...
		$sql = "UPDATE movimientobn SET status='0' WHERE id_mov='".$idDesin."'";
		$this->ejecuta($sql);

		if( $this->como_va() ){
			/* inserto los datos correspondiente de la anulacion en la tabla movimientos */
			$sql = "UPDATE movimientobn SET id_usuario_anulacion='".$_SESSION["id_usuario"]."', fecha_anulacion='".$fecha."', id_motivo_anulacion='".$MotAnulacion."' WHERE id_mov='".$idDesin."'";
			$this->ejecuta($sql);

			if( $this->como_va() ){

				/* actulizo el status en la tabla detalle del movimiento */
				$sql = "UPDATE dmovimientobn SET status='0' WHERE id_mov='".$idDesin."'";
				$this->ejecuta($sql);
				...
				if( $this->como_va() ){
					...
					$sql = "SELECT id_bien FROM dmovimientobn WHERE id_mov='".$idDesin."'";
					$rs = $this->ejecuta($sql);
					if( $this->como_va() ){
	...
	function consultar_desincorporacion_bitacora()
	{
		$this->conectar();
		$cont=0;
		$sql = "SELECT
		...
		WHERE mov.id_mov='$this->lcIDdocumento' AND mov.status='1' AND arti.status='1' AND dmov.status='1' AND mov.id_tipo_mov='4'
